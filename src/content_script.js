...
// Use the following patterns to check for on-screen LinkedIn elements
...
const LOGIN_PATTERN_DETECTION_SELECTORS = [
  ...
  "[class*='LinkedInConnectButton']",
  "[class*='js-linkedin-login']", // kickstarter
  ".signup__button.button--linkedin", // massdrop
  "[id*='signin_fb_btn']", // Ebay
  ".btn-social.linkedin", // emag.ro
  ".button-login.linkedin-login", // estadao.com.br
  "[class*='signup-provider-linkedin']", // Fandom
  ".socialContainer-0-131 .mainLinkedIn-0-150", // Honey
  "[class*='linkedin_login_click']", // Hi5
  "[data-test-id*='linkedin-create-button']", // Doordash
  "[class*='LinkedInButton--linkedin-button']", // Strava
  "#login_form > .linkedin", // Strava
  "[class*='linkedin-connect-button']", // Twitch
  "[data-testid*='linkedin-login']", // Spotify
  ".logInWithButtons .logInWith.linkedin", // bazqux.com
  "[href*='signin/linkedin']",
  "[data-test*='login-with-linkedin']", // Producthunt
  "[data-oauthserver*='linkedin']", // Stackoverflow
  ".puppeteer_test_login_button_linkedin", // Quora
  "[href*='connect/linkedin']", //Medium
  "[data-login-with-linkedin='']", // etsy
  "[data-destination*='linkedin']",
  ".fm-sns-item.linkedin", // AliExpress
  ".social-login .button--linkedin", // noovie.com
  "#home_account_fb", // Deezer
  ".front-door .btn.btn-linkedin", // GroupMe
  "[class*='meetup-signupModal-linkedin']", // Meetup Signup Homepage
  "#linkedin-register", // Meetup Signup Static Page
  "[href*='https://www.linkedin.com/v11.0/dialog/oauth']", // Meetup Signup Non-homepage
  ".fb-start .ybtn--social.ybtn--linkedin", // Yelp
  "[aria-label*='Log in with LinkedIn']", // Tinder
  ".registration__form .button.color-provider-linkedin", // Bumble
  "#linkedinLoginButton", // eHarmony
  "[action*='linkedin_login']", // Airbnb
  "[action*='linkedin_signup']", // Airbnb
  "#social-auth-provider-linkedin-web", // VRBO
  "[class*='FBLoginForm__Button']", // Mixcloud homepage
  ".button.button--linkedin", // Buzzfeed Login
  "#js-linkedin-oauth-login", // NY Times
  "#login-linkedin-button", // Indeed
  ".btn-social-connect.btn-linkedin", // Zillow (Zindex Issue)
  "[class*='fb-login']", // Default FB class name "lic-login-button"
  ".fb-login-button" // Default FB class name "lic-login-button"
];

// TODO: Disarm click events on detected elements
const SHARE_PATTERN_DETECTION_SELECTORS = [
  "[href*='linkedin.com/dialog/feed']", // Feed dialog
  "[data-bfa-network*='linkedin']", // Buzzfeed Mini Share
  "[aria-label*='share on linkedin']", // MSN
  "[data-tracking*='linkedin|share']", // football.london
  "[class*='linkedinShare']", // Producthunt share
  "[class*='social-tray__link--linkedin']", // Vice
  ".post-action-options + .right > .social-icon.icon-f", // Imgur share
  "[title='Share on LinkedIn']" // Medium
];

// TODO: Disarm click events on detected elements
const PASSIVE_SHARE_PATTERN_DETECTION_SELECTORS = [
  "[href*='linkedin.com/dialog/share']", // Share dialog
  "[href*='linkedin.com/sharer']", // Legacy Share dialog
];
...
const fragmentClasses = ["lic-badge-fence", "lic-badge-tooltip", "lic-badge-prompt"];
...
function createBadgeFragment(socialAction) {
  ...
  // Create Tooltip
  const htmlBadgeFragmentTooltipDiv = htmlBadgeFragment.querySelector(".lic-badge-tooltip");
...
function buildInpageIframe(socialAction, fbcIframeHeight) {
  ...
  iframe.title = browser.i18n.getMessage("linkedinContainer");
  iframe.tabIndex = 0;
  iframe.ariaHidden = "false";
  iframe.id = socialAction;
  iframe.classList.add("lic-content-box");
...
function positionIframe(fencePos) {
  ...
  const iframeBox = document.querySelector(".lic-content-box");
  const iframeWrapper = document.querySelector(".lic-wrapper");
  const iframeElement = iframeWrapper.getElementsByTagName("iframe");
  const iframeChevron = document.querySelector(".lic-iframe-chevron");
...
function mobileOrientation(iframeElement, iframeChevron, iframeBox, fencePosition, offsetY){
  ...
  iframeChevron.classList.add("lic-chevron-arrow-top");
...
function desktopOrientation(iframeBox, iframeChevron, offsetY, fencePosition, iframePaddingAllowance, fencePos, offsetX) {
  ...
  iframeChevron.classList.remove("lic-chevron-arrow-top");
  ...
  if (iframePaddingAllowance > calculateOffsetDiff) {
    ...
    iframeChevron.classList.add("lic-chevron-arrow-right");
    iframeChevron.style.marginLeft = `${xPosChevron - fencePos.offsetWidth - iframeChevron.offsetWidth - offsetX}px`;
    return;
  }
  return iframeChevron.classList.remove("lic-chevron-arrow-right");
}
...
function openInputPrompt(socialAction, fencePos, target, fbcIframeHeight) {
  ...
  const hasFbcWrapper = document.querySelector(".lic-wrapper");
  ...
  if (!hasFbcWrapper) {
    ...
    ["resize", "scroll"].forEach(function (evt) {
      if (document.querySelector(".lic-wrapper")) {
...
function addLinkedInBadge(target, badgeClassUId, socialAction) {
  // Detect if target is visible

  const htmlBadgeDiv = createBadgeFragment(socialAction);

  const htmlBadgeFragmentFenceDiv = htmlBadgeDiv.querySelector(".lic-badge-fence");

  htmlBadgeDiv.className = "lic-badge " + badgeClassUId;
  ...
  if (badgeSmallSwitch) {
    htmlBadgeDiv.classList.add("lic-badge-small");
  }
  ...
  case "email":
    ...
    break;
  case "share-passive":
    htmlBadgeDiv.classList.add("lic-badge-share-passive", "lic-badge-share");
    shareBadgeEventListenerInit(target, htmlBadgeDiv, { allowClickThrough: true });
    break;
  case "share":
    htmlBadgeDiv.classList.add("lic-badge-share");
    shareBadgeEventListenerInit(target, htmlBadgeDiv, { allowClickThrough: true });
    break;
  } 
  ...
  positionLinkedInBadge(target, badgeClassUId, itemWidth, badgeSmallSwitch);
}
...
function shareBadgeEventListenerInit(target, htmlBadgeDiv, options) {
  ...
  target.addEventListener("mouseover", () => {
    target.classList.add("lic-badge-tooltip-active");
    htmlBadgeDiv.classList.add("lic-badge-tooltip-active");
    setTimeout(() => {
      positionPrompt(htmlBadgeDiv);
    }, 50);
  });
  ...
  target.addEventListener("mouseout", () => {
    target.classList.remove("lic-badge-tooltip-active");
    htmlBadgeDiv.classList.remove("lic-badge-tooltip-active");
  });
}
...
function positionPrompt(activeBadge) {
  ...
  if ((window.innerWidth - elemRect.left) < 350) {
    activeBadge.classList.add("lic-badge-prompt-align-right");
  }

  const modifierClassList = ["lic-badge-prompt-align-top", "lic-badge-prompt-align-bottom", "lic-badge-prompt-align-right"];

  if (elemRect.top < 140) {
    activeBadge.classList.add("lic-badge-prompt-align-top");
  } else if ((window.innerHeight - elemRect.bottom) < 130) {
    activeBadge.classList.add("lic-badge-prompt-align-bottom");
...
function getOffsetsAndApplyClass(elemRect, bodyRect, target, htmlBadgeDiv) {
  if (!isFixed(target) && htmlBadgeDiv.classList.contains("lic-badge-fixed")) {
    htmlBadgeDiv.classList.remove("lic-badge-fixed");
  } else if (isFixed(target)) {
    htmlBadgeDiv.classList.add("lic-badge-fixed");
...
function checkVisibilityAndApplyClass(target, htmlBadgeDiv) {
  ...
  if (target === null) {
    htmlBadgeDiv.classList.add("lic-badge-disabled");
    return false;
  }

  const htmlBadgeDivHasDisabledClass = htmlBadgeDiv.classList.contains("lic-badge-disabled");
  ...
  if (parentElement) {
    if (!isVisible(parentElement)) {
      ...
      if (htmlBadgeDivHasDisabledClass) {
        htmlBadgeDiv.classList.remove("lic-badge-disabled");
      }
      return true;
    }
  }
...
function positionLinkedInBadge(target, badgeClassUId, targetWidth, smallSwitch) {
  ...
  if (typeof smallSwitch === "undefined") {
    if (htmlBadgeDiv.classList.contains("lic-badge-small")) {
      smallSwitch = true;
    }
  }
...
function isPinterest(target) {
  ...
  if (parentElement) {
    ...
    if (previousElementSibling) {
      return previousElementSibling.classList.contains("lic-has-badge");
    }
  }
  return false;
}
...
function parentIsBadged(target) {
  const { parentElement } = target;
  if (parentElement) {
    return parentElement.classList.contains("lic-has-badge");
  }
  return false;
}

// List of badge-able in-page elements
const linkedinDetectedElementsArr = [];
...
function patternDetection(selectionArray, socialActionIntent){
  ...
  for (let item of document.querySelectorAll(querySelector)) {
    ...
    if (!item.classList.contains("lic-has-badge") && !isPinterest(item) && !parentIsBadged(item)) {
      const itemUIDClassName = "lic-UID_" + (linkedinDetectedElementsArr.length + 1);
      const itemUIDClassTarget = "js-" + itemUIDClassName;
      const socialAction = socialActionIntent;
      linkedinDetectedElementsArr.push(itemUIDClassName);
      addLinkedInBadge(item, itemUIDClassTarget, socialAction);
      item.classList.add("lic-has-badge");
...
async function detectLinkedInOnPage () {
  if (!checkForTrackers) {
    return;
  }
...
function screenUpdate() {
  if (checkForTrackers) {
    for (let item of linkedinDetectedElementsArr) {
      positionLinkedInBadge(item);
    }
  }
}
...
function escapeKeyListener() {
  document.body.addEventListener("keydown", function (e) {
    if (e.key === "Escape" && document.querySelector(".lic-wrapper")) {
      closeIframe();
    }
  });
}
...
window.addEventListener("click", function () {
  if (this.document.querySelector(".lic-wrapper")) {
    closeIframe();
  }
});
...
function removeBadges() {
  for (let itemClass of linkedinDetectedElementsArr) {
    // positionLinkedInBadge(item);
    const target = document.querySelector("." + itemClass);
    target.classList.remove("lic-has-badge");
...
browser.runtime.onMessage.addListener(message => {
  if (message["msg"] == "allowed-linkedin-subresources" || message["msg"] == "linkedin-domain") {
