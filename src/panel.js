...
// adds "LinkedIn Container" to top of all panels
const addHeader = (wrapper) => {
  const el = document.createElement("h1");
  el["id"] = "linkedInContainer";
...
// creates grey LinkedIn text. Grey fence icon is set in CSS.
const addLinkedInAndIcon = async () => {
  const el = document.createElement("p");
  el.innerText = "LinkedIn";
  const browserInfo = await browser.runtime.getBrowserInfo();
  if (parseInt(browserInfo.version) < 67) {
    el.classList.add("LinkedIn-blue-text");
    return el;
  }
  el.classList.add("LinkedIn-text");
  return el;
};
...
const buildPanel = async(panelId) => {
  ...
  if (panelId === "on-linkedin") {
    let el = await addLinkedInAndIcon(contentWrapper);
  ...
  if (["trackers-detected", "in-lic"].includes(panelId)) {
...
const buildOnboardingPanel = async (panelId) => {
  ...
  if (panelId === 2) {
    let el = await addLinkedInAndIcon();
...
const addSiteToContainer = async () => {
  ...
  fbcStorage.domainsAddedToLinkedInContainer.push(activeRootDomain);
  await browser.storage.local.set({"domainsAddedToLinkedInContainer": fbcStorage.domainsAddedToLinkedInContainer});
  browser.tabs.reload();
  window.close();
};
